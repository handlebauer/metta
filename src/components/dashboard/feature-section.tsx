import {
    IconAi,
    IconBrandIntercom,
    IconChartBar,
    IconHeartHandshake,
    IconMessageChatbot,
    IconRobot,
    IconTicket,
    IconUsers,
} from '@tabler/icons-react'

import { cn } from '@/lib/utils'

export function FeaturesSection() {
    const features = [
        {
            title: 'AI-Powered Support',
            description:
                'Leverage advanced AI to handle customer inquiries automatically and intelligently.',
            icon: <IconAi className="h-6 w-6" />,
        },
        {
            title: 'Multi-Channel Support',
            description:
                'Seamlessly integrate with email, chat, and social media platforms.',
            icon: <IconBrandIntercom className="h-6 w-6" />,
        },
        {
            title: 'Smart Ticket Management',
            description:
                'Automatically categorize, prioritize, and route support tickets.',
            icon: <IconTicket className="h-6 w-6" />,
        },
        {
            title: 'Analytics & Insights',
            description:
                'Get detailed insights into your customer support performance.',
            icon: <IconChartBar className="h-6 w-6" />,
        },
        {
            title: 'Customer Self-Service',
            description:
                'Empower customers with AI-powered self-service tools.',
            icon: <IconRobot className="h-6 w-6" />,
        },
        {
            title: '24/7 AI Assistance',
            description:
                'Our AI agents are always available to help your customers.',
            icon: <IconMessageChatbot className="h-6 w-6" />,
        },
        {
            title: 'Team Collaboration',
            description:
                'Streamline communication between support teams and departments.',
            icon: <IconUsers className="h-6 w-6" />,
        },
        {
            title: 'Customer Success',
            description: 'Build stronger relationships with proactive support.',
            icon: <IconHeartHandshake className="h-6 w-6" />,
        },
    ]
    return (
        <div className="relative z-10 mx-auto grid max-w-7xl grid-cols-1 py-10 md:grid-cols-2 lg:grid-cols-4">
            {features.map((feature, index) => (
                <Feature key={feature.title} {...feature} index={index} />
            ))}
        </div>
    )
}

const Feature = ({
    title,
    description,
    icon,
    index,
}: {
    title: string
    description: string
    icon: React.ReactNode
    index: number
}) => {
    return (
        <div
            className={cn(
                'group/feature relative flex flex-col py-10 dark:border-neutral-800 lg:border-r',
                (index === 0 || index === 4) &&
                    'dark:border-neutral-800 lg:border-l',
                index < 4 && 'dark:border-neutral-800 lg:border-b',
            )}
        >
            {index < 4 && (
                <div className="pointer-events-none absolute inset-0 h-full w-full bg-gradient-to-t from-neutral-100 to-transparent opacity-0 transition duration-200 group-hover/feature:opacity-100 dark:from-neutral-800" />
            )}
            {index >= 4 && (
                <div className="pointer-events-none absolute inset-0 h-full w-full bg-gradient-to-b from-neutral-100 to-transparent opacity-0 transition duration-200 group-hover/feature:opacity-100 dark:from-neutral-800" />
            )}
            <div className="relative z-10 mb-4 px-10 text-neutral-600 dark:text-neutral-400">
                {icon}
            </div>
            <div className="relative z-10 mb-2 px-10 text-lg font-bold">
                <div className="absolute inset-y-0 left-0 h-6 w-1 origin-center rounded-br-full rounded-tr-full bg-neutral-300 transition-all duration-200 group-hover/feature:h-8 group-hover/feature:bg-blue-500 dark:bg-neutral-700" />
                <span className="inline-block text-neutral-800 transition duration-200 group-hover/feature:translate-x-2 dark:text-neutral-100">
                    {title}
                </span>
            </div>
            <p className="relative z-10 max-w-xs px-10 text-sm text-neutral-600 dark:text-neutral-300">
                {description}
            </p>
        </div>
    )
}
